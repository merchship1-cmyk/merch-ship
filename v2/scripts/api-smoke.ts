...
if (!apiKey) {
  throw new Error('OPENAI_API_KEY is required for the main-branch API smoke test.');
}
...
const response = await fetch('https://api.openai.com/v1/responses', {
  ...
  body: JSON.stringify({
    ...
    input: 'Turn a messy weekly planning process into one clear execution loop.',
    text: {
      format: {
        type: 'json_schema',
...
console.log('Responses API structured-output smoke test passed with ' + model + '.');
